...
def test_parse_rss_urls_normalizes_and_deduplicates_links() -> None:
    ...
    <rss version="2.0">
      <channel>
        <item><link>https://example.org/reports/latest/?b=2&amp;a=1#summary</link></item>
    ...
    assert parse_feed_urls(
        content,
        document_url="https://example.org/feed.xml",
    ) == ("https://example.org/reports/latest?a=1&b=2",)
...
def test_parse_atom_urls_uses_alternate_links_and_resolves_relative_urls() -> None:
    ...
    <feed xmlns="http://www.w3.org/2005/Atom">
      ...
      <entry><link href="https://example.org/reports/2/" /></entry>
    </feed>
    """

    assert parse_feed_urls(
        content,
        document_url="https://example.org/feeds/news.xml",
    ) == (
...
def test_parse_rss_one_urls() -> None:
    ...
    <rdf:RDF
      ...
      <item><link>https://example.org/reports/1</link></item>
    </rdf:RDF>
    """

    assert parse_feed_urls(
        content,
        document_url="https://example.org/feed.rdf",
    ) == ("https://example.org/reports/1",)
...
def test_parse_sitemap_urlset_returns_only_content_urls() -> None:
    ...
    <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
      <url><loc>https://example.org/reports/1</loc></url>
    ...
    assert parse_sitemap_urls(
        content,
        document_url="https://example.org/sitemap.xml",
    ) == SitemapDiscovery(
        content_urls=(
            "https://example.org/reports/1",
            "https://example.org/reports/2",
...
def test_discovery_rejects_invalid_or_unsafe_documents(
    ...
    with pytest.raises(DiscoveryParseError, match=message):
        parser(content, document_url="https://example.org/discovery.xml")
