"""LLM-backed scratch rewriter implementation."""
...
SYSTEM_PROMPT = (
    "Maintain a compact research scratch for a multi-step research agent. Rewrite the "
    "entire scratch using the supplied current scratch, research history, and complete "
    ...
    '{"content":"<complete replacement scratch>"}'
)


class LLMScratchRewriter:
    """Rewrite scratch state with an LLM only when the action is executed."""
    ...
    async def rewrite(self, request: ScratchRewriteRequest) -> ScratchRewriteResult:
        response = await self.client.generate(
            LLMRequest(
                ...
                metadata={"trace": {"operation": "scratch_rewriter.rewrite"}},
            )
        )
        return ScratchRewriteResult.model_validate_json(response.content)

    @staticmethod
    def _build_user_prompt(request: ScratchRewriteRequest) -> str:
        return json.dumps(
            {
                "user_query": {
                    "text": request.query,
                ...
                "current_scratch": request.current_scratch,
